...
## FastText support 
FastText is preinstalled in the image and the `lid.176.bin` model is downloaded automatically. Because the image wires these values internally, providing `langtool_fasttextModel` or `langtool_fasttextBinary` environment variables causes the container to exit with an error. 
